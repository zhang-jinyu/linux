...
static int adrv9002_mcs_status_show(struct seq_file *s, void *ignored)
{
	...
	mutex_lock(&phy->lock);
	ret = adi_adrv9001_Mcs_Status_Get(phy->adrv9001, &mcs_status);
	mutex_unlock(&phy->lock);
	if (ret)
		return adrv9002_dev_err(phy);
	/* RF1 PLL */
...
static int adrv9002_tx_mcs_strobe_delay_get(void *arg, u64 *val)
{
	...
	mutex_lock(&phy->lock);
	ret = adi_adrv9001_Mcs_TxMcsToStrobeSampleLatency_Get(phy->adrv9001,
...
static int adrv9002_mcs_delays_show(struct seq_file *s, void *ignored)
{
	...
	mutex_lock(&phy->lock);
	ret = adi_adrv9001_Mcs_ChannelMcsDelay_Get(phy->adrv9001, c->port, c->number,
						   &mcs_status);
	mutex_unlock(&phy->lock);
	if (ret)
		return adrv9002_dev_err(phy);
