...
/* Sampling timing */
#define AD4630_FREQ_TO_PERIOD(f)	\
	DIV_ROUND_UP(USEC_PER_SEC, ((f) / NSEC_PER_USEC))
...
// static const unsigned int ad4630_sampling_rates[] = {
...
// static ssize_t ad4630_sampling_freq_avail(struct device *dev,
...
// 	for (i = 0; i < ARRAY_SIZE(ad4630_sampling_rates); i++)
// 		len += scnprintf(buf + len, PAGE_SIZE - len, "%d ",
// 				 ad4630_sampling_rates[i]);
// 	buf[len - 1] = '\n';

// 	return len;
// }
// static IIO_DEV_ATTR_SAMP_FREQ_AVAIL(ad4630_sampling_freq_avail);
...
static int ad4630_set_sampling_freq(struct ad4630_state *st, unsigned int freq)
{
	...
	unsigned long long target, period_ps;
...
// 	&iio_dev_attr_sampling_frequency_available.dev_attr.attr, NULL
// };

// static const struct attribute_group ad4630_group = {
...
static int ad4630_probe(struct spi_device *spi)
{
	...
	if (device_id == ID_AD463X) {
		...
		for (i = 0; i < ARRAY_SIZE(ad4630_id_table); i++) {
			if (ad4630_id_table[i].driver_data == device_id) {
	...
	indio_dev->channels =
		ad4630_ch_info[device_id].channels[st->phy.out_data_mode];
	indio_dev->num_channels = ad4630_ch_info[device_id].channels_number;
